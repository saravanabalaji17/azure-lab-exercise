...
3. In the search bar, type **Virtual machines** → **Create → Azure Virtual Machine**
   ...
   * **Size:** Standard\_B1s (free tier eligible) (OR) Standard_D2s_v3
...
1. In Azure Portal, go to **Virtual machines** → **Create → Azure Virtual Machine**
   ...
   * **Image:** *Windows Server 2022 Datacenter – x64 Gen2 (OR) Windows Server 2025 Datacenter – x64 Gen2*
   * **Size:** Standard\_B2s (or free tier eligible if available) (OR) Standard_D2s_v3
   * **Authentication type:** Password
...
* **Linux VM** → run:
  ...
  whoami
  hostname
  yum install net-tools wget zip -y
  ifconfig
  curl ifconfig.me
  ```
